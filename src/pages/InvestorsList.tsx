...
//         `http://localhost:9000/api/admin/testing/${gameId}/investors?sort=${sort}`,
//         {
//           headers: {
//             'Content-Type': 'application/json',
//             Authorization: `Bearer ${adminToken}`,
...
const InvestorList: React.FC = () => {
  ...
  const fetchInvestors = async (gameId: string, sort: 'lowest' | 'highest' = 'lowest') => {
    try {
      setLoading(true);
      const response = await fetch(
        `https://satashreejibackend.onrender.com/api/admin/testing/${gameId}/investors?sort=${sort}`,
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${adminToken}`,
  ...
  investor.betHistory.map((bet, idx) => (
    <tr key={`${investor.userId}-${idx}`}>
      <td>
        <div className="investor-info">
          <div className="investor-avatar">
            {investor.profileImage ? (
      ...
      <td>
        <div className="bet-details">
          {bet.betNumbers.map((num) => (
            <div key={num._id} className="bet-number">
              <TrendingUp size={14} />
              Number: {num.number} - ₹{num.amount}
            </div>
          ))}
          <div className={`bet-type ${getBetTypeColor(bet.betType)}`}>
            {bet.betType}
          </div>
        </div>
      </td>
      ...
      <td>
        <div className="amount-info">
          <span className="amount">₹{bet.totalBetAmount}</span>
        </div>
      </td>

      <td>
        <div className={`status-badge ${getStatusColor(bet.status)}`}>
          {bet.status}
        </div>
      </td>
