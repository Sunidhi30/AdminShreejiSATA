...
const [declaringGame, setDeclaringGame] = useState<Game | null>(null);
const [resultDigit, setResultDigit] = useState<number>(0);
  ...
  const handleDeclareResult = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!declaringGame) return;
  
    try {
      const response = await fetch(`https://satashreejibackend.onrender.com/api/admin/admin/hardgame/declare`, {
        method: 'POST',
        headers: getHeaders(),
        body: JSON.stringify({
          gameId: declaringGame._id,
          resultDigit
        })
      });
      ...
      if (response.ok) {
        setSuccess(data.message || 'Result declared successfully');
        setShowDeclareModal(false);
        setDeclaringGame(null);
        fetchGames();
      } else {
        setError(data.message || 'Failed to declare result');
      }
    } catch (err) {
      setError('Error declaring result: ' + (err as Error).message);
    }
  };
            ...
            <tbody>
              {games.map((game) => (
                <tr key={game._id}>
                  ...
                  <td>{game.lastResultTime ? formatDate(game.lastResultTime) : 'N/A'}</td>
                  <td>
  <button className="admin-games__edit-btn" onClick={() => openEditModal(game)}>Edit</button>
  <button
    className="admin-games__edit-btn"
    style={{ marginLeft: '8px', backgroundColor: '#F4B400', color: '#000' }}
    onClick={() => {
      setDeclaringGame(game);
      setResultDigit(0);
  ...
  >
    Declare
      ...
      {showDeclareModal && declaringGame && (
  <div className="admin-games__modal-overlay">
    <div className="admin-games__modal">
      <div className="admin-games__modal-header">
        <h2>Declare Result</h2>
        <button className="admin-games__close-btn" onClick={() => {
          setShowDeclareModal(false);
          setDeclaringGame(null);
        }}>×</button>
      </div>
      <form onSubmit={handleDeclareResult} className="admin-games__form">
        <div className="admin-games__form-group">
          <label>Game: <strong>{declaringGame.gameName}</strong></label>
        </div>
        <div className="admin-games__form-group">
          <label>Result Digit (0-9) *</label>
          <input
            type="number"
            min="0"
            max="9"
            value={resultDigit}
            onChange={(e) => setResultDigit(Number(e.target.value))}
            required
          />
        </div>
        <div className="admin-games__form-actions">
          <button
            ...
            className="admin-games__cancel-btn"
          >
            Cancel
          </button>
          <button type="submit" className="admin-games__submit-btn">
