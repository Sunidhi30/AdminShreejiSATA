...
const GameResult: React.FC = () => {
  ...
  return (
    <div className="game-result-container">
      <div className="game-result-header">
        <h1>Game Results</h1>
        <button className="declare-btn" onClick={() => setShowModal(true)}>Declare Result</button>
      </div>
        ...
        <>
                ...
                {results.length === 0 ? (
                  ...
                  results.map(result => (
                    <tr key={result._id}>
                      <td>{formatDateTime(result.date)}</td>
                      <td>{result.gameId.name}</td>
                      <td>{result.openResult !== undefined ? result.openResult : '-'}</td>
                      <td>{result.closeResult !== undefined ? result.closeResult : '-'}</td>
                      <td>{formatDateTime(result.createdAt)}</td>
                    </tr>
                  ))
                )}
          ...
          {pagination.pages > 1 && (
            <div className="pagination">
              ...
              {Array.from({ length: pagination.pages }, (_, i) => i + 1).map(page => (
                <button key={page} onClick={() => handlePageChange(page)} className={pagination.current === page ? 'active' : ''}>
                  {page}
                </button>
              ))}
              <button onClick={() => handlePageChange(pagination.current + 1)} disabled={pagination.current === pagination.pages}>
                Next
              </button>
            </div>
          )}
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={declareResult} className="declare-form">
              ...
              <div className="form-group">
                <label htmlFor="closeResult">Close Result (0-99):</label>
                <input
                  type="number"
                  id="closeResult"
                  name="closeResult"
